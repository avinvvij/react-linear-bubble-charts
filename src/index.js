...
class LinearBubbleChart extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            wordWrap: this.props.wordWrap ? this.props.wordWrap : false
        }
    }
    ...
    makeWordWrapContent = (sentence, circle_radius, sentence_index) => {
        let sentence_split_arr = sentence.split(" ");
        return sentence_split_arr.map((word, index) => {
            return (<tspan x={(45 - (word.toString().length * 2)) + (sentence_index * this.state.bubbleSpacing)} y={(this.state.height / 2 + circle_radius) + (20 * (index + 1))}>{word}</tspan>)
        })
    }
                        ...
                        this.props.data.map((data_item, index) => {
                            ...
                            return (
                                <g key={index}>
                                    ...
                                    {
                                        this.state.wordWrap &&
                                        <text fill={this.props.valueColor ? this.props.valueColor : "black"} >{
                                            this.makeWordWrapContent(data_item.title, circle_radius, index)
                                        }</text>
                                    }
                                    {!this.state.wordWrap && <text fill={this.props.titleColor ? this.props.titleColor : "black"} x={(45 - (data_item.title.length * 2)) + (index * this.state.bubbleSpacing)} y={this.state.height / 2 + circle_radius + 20}>{data_item.title}</text>}
                                </g>
                            )
                        })
                    }
