...
class LinearBubbleChart extends Component{
    ...
    getDataMaxValue(){
        ...
        if(this.props.data){
            ...
            value_interval = (max_value - min_value)/(max_bubble_size-min_bubble_size)
        }
        return value_interval
    }

    render(){
        let value_interval = getDataMaxValue();
        return(
            <div>
                <svg height = {this.state.height} width={100+(this.props.data.length * (this.state.bubbleSpacing))}>
                    <path stroke = "white" d={"M 10 "+this.height/2+"L "+((this.props.data.length * this.state.bubbleSpacing)+20) +" "+this.state.height/2}></path>
                    {
                        this.props.data.map((data_item , index)=>{
                            ...
                            return(
                            <g key={index}>                                
                                <circle onClick={this.props.onBubbleClick?()=>{this.props.onBubbleClick(index , data_item.value , data_item.title)}:null} cx={50+(index*this.state.bubbleSpacing)} cy={this.height/2} r={circle_radius} fill={this.props.bubbleColor? this.props.bubbleColor: "#FB6669"}></circle>
                                <text fill={this.props.valueColor? this.props.valueColor:"black"} x={45+(index*this.state.bubbleSpacing)} y= {this.state.height/2 - circle_radius - 20}>{data_item.value}</text>
                                <text fill={this.props.titleColor?this.props.titleColor:"black"} x={45+(index*this.state.bubbleSpacing)} y= {this.state.height/2 + circle_radius + 20}>{data_item.title}</text>
                            </g>
                            )
                        })
                    }
